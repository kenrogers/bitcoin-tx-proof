...
describe("Clarity Bitcoin Lib v5 verification", () => {
  ...
  test("confirm clarity remote data", async () => {
    ...
    expect(bytesToHex((bbhh.result as SomeCV<BufferCV>).value.buffer)).toBe(
      bitcoinBlockHeaderHash
    );
  });
  ...
  test("verify tx using Stacks", async () => {
    ...
    const proof = fromRemote
      ? await bitcoinTxProof(txId, blockHeight, rpcConfig)
      : cachedProof;
    ...
    expect((legacyTxVerified.result as ResponseOkCV).value.type).toBe(
    ...
    expect((segwitTxVerify.result as ResponseOkCV).value.type).toBe(
      ClarityType.ResponseOk
    );
  }, 100_000);
});
