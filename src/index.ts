...
function verifyMerkleProof(
  ...
  let currentHash: Buffer<ArrayBufferLike> = Buffer.from(txid, "hex").reverse();
  ...
  if (calculatedRoot !== merkleRoot) {
    throw new Error(
      `Merkle proof verification failed: Expected ${merkleRoot} but got ${calculatedRoot}`
    );
  }
...
export async function bitcoinTxProof(
    ...
    try {
      ...
      const txs = await Promise.all(
        block.tx.map(async (tx: any) => {
          const rawTxData = await rpc.call("getrawtransaction", [
            tx.txid,
            true,
            blockHash,
          ]);
          const hasWitness =
            rawTxData.hex.includes("0001") ||
            rawTxData.vin.some((input: any) => input.txinwitness);
          debug(`Transaction ${tx.txid}:`, {
            hasWitness,
            vinLength: rawTxData.vin.length,
            hasWitnessData: rawTxData.vin.some(
              (input: any) => input.txinwitness
            ),
          });
      ...
      const { proof, root: calculatedRoot } = calculateWitnessMerkleProof(
        txs,
        txIndex
      );
      ...
      if (proof.length === 0) {
        ...
        witnessMerkleProof = proof
          .map((step) => step.data.toString("hex"))
          .join("");
        witnessMerkleProofArray = proof.map(
          (step) => new Uint8Array(step.data)
        );
  ...
  const result: TxProofResult = {
    ...
    coinbaseMerkleProofArray: merkleProof.map(
      (step) => new Uint8Array(step.data)
    ),
  };
